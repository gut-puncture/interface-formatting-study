...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" || $# -lt 3 ]]; then
  echo "Usage: $0 <user@host> <model-slug> <semantic-run-id> [remote_dir] [local_dir] [ssh_key] [ssh_port] [complete|partial]"
  exit 0
fi
...
DEST="${LOCAL_DIR}/${MODEL_SLUG}/${SEMANTIC_RUN_ID}/$(date -u +%Y%m%dT%H%M%SZ)"
mkdir -p "$DEST"

rsync -az -e "${RSYNC_RSH[*]}" \
  "${REMOTE}:${REMOTE_DIR}/results/causal_runs/${MODEL_SLUG}/${SEMANTIC_RUN_ID}/" \
  "${DEST}/"
